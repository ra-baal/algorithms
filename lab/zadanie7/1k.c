...
void wyswietl_el( el* e ) {
    if ( ! e )
        ...
        printf("adres obiektu: %p | x: %2i nast: %p\n" , e , e->x , e->nast);
}
...
//// zadanie 3
int ile_wystapien (el* poczatek, const int x) {
/* Zwraca liczbę wystąpień x-a w jednokierunkowej liście.
 * Funkcja napisana jako pomocnicza dla funkcji 'int max_powt(el* poczatek)'*/

    int lx; // liczba wystąpień

    for ( lx = 0 ; poczatek ; poczatek = poczatek->nast )
        if ( poczatek->x == x )
            ++lx;
...
int max_powt(el* p) {
/* Zwraca najczęściej powtarzającą się wartość z listy. */

    int x, lx; // wartość x, liczba x-ów w liście - ta wartoć zostanie zwrócona
    int y, ly; // wartość y, liczba y-ów w liście

    if (! p) {
        fprintf(stderr, "int max_powt(%p): Lista nie moze byc pusta.\n", p);
        exit(1); // lista pusta
    }

    x = p->x; // bierzemy pierwszą wartość
    lx = ile_wystapien(p, x); // i liczbę jej wystąpień
    while (p->nast) {

        y = p->nast->x; // bierzemy następną wartość
        ly = ile_wystapien(p, y); // i liczbę jej wystąpień
...
/* Funkcja usuwająca z listy l wszystkie liczby niepodzielne przez k*/

    el* q; // wsk pomocniczy
    el* zwol;

    if ( k == 0) {
        printf("Nie mozna dzielic przez k == 0\n");
        return;
    }
    ...
    /* Usuwanie z początku listy */
    while ( (*l) && ((*l)->x % k != 0) ) { /* dopóki niepusta i element do usunięcia jest na początku */
        zwol = (*l)->nast;
        free(*l);
        *l = zwol;
    }
    ...
    while (q && q->nast) {
        if ( (q->nast)->x % k != 0 ) {
            zwol = q->nast; // do zwolnienia

            q->nast = (q->nast)->nast;

            free(zwol); // zwolnienie
        } else {
            q = q->nast;
        }
    }
