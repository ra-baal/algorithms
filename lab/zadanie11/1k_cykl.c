...
/// zadanie 11
void usun_co_k_cykl(el1k** L , const int k) {
/* usuwa z listy co k-ty element, aż zostanie tylko jeden. */
    int i;
    el1k* poprz;
    ...
    if (k < 1) {
        printf("k nie moze byc mniejsze od 1\n");
        return;
    }

    if (k == 1) {
        // dla k = 1 potrzeba znać poprzedni pierwszego elementu (czyli ostatni)
        // bo będzie on usuwany (pierw. el.)
        poprz = *L;
        while ( poprz->nast != *L )
            poprz = poprz->nast;
    }
