...
/* A -> A \ B // z listy A usuwa elementy występujące również w B
 * B -> B \ A // z listy B usuwa elementy występujące również w A */

    int usuwany_x;

    el* p = *A; // wsk obecnego elementu na liście A
    el* p_poprz = NULL; // poprzedni element względem p

    el* q = *B; // wsk obecnego elementu na liście B
    el* q_poprz = NULL; // poprzedni element względem q
    ...
    while (p && q) {
        ...
        if (q) while (p && p->x < q->x) {
                p_poprz = p;
                p = p->nast;
            }

        if (p) while (q && q->x < p->x) {
                q_poprz = q;
                q = q->nast;
            }
    }
