...
/*! Pomocnicza dla funkcji zapis_listy. Rekurencyjna. */
void drukuj_do_pliku_rek(FILE * plik, drzewo d) {
    if (!d)
        return;

    // przechodzenie rodzic-lewy-prawy i pisanie do pliku
    fprintf( plik , "%s\n" , d->lancuch );
    drukuj_do_pliku_rek(plik, d->lewy);
    drukuj_do_pliku_rek(plik, d->prawy);
}

/*! Zapisuje drzewo do pliku.
 * @return Zwraca: 1 - powodzenie, 0 - niepowodzenie */
int zapis_drzewa( drzewo d , char* nazwa_pliku ) {
    ...
    drukuj_do_pliku_rek(plik, d);

    fclose(plik);
    return 1; // zwraca prawdę - powodzenie
}

/*! Wczytuje drzewo z pliku dodajac je do obecnego.
 * @return Zwraca: 1 - powodzenie, 0 - niepowodzenie */
int wczytanie_drzewa( drzewo * d , char* nazwa_pliku ) {
